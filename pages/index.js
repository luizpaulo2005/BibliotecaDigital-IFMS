  ...
  return (
    ...
    <div id="carouselExampleCaptions" className="carousel slide carrossel border rounded" data-bs-ride="false">
        ...
        {attributes.map(({id, titulo, discente}) =>(
          ...
          <Image src={"/images/gray.png"} alt="Image" width={800} height={170}/>
          </div>
          <div className="carousel-caption d-none d-md-block">
          <h5><Link href={`/posts/user/solo/pesquisa/${id}`}><a className='list-group-item'>{titulo}</a></Link></h5>
          <p>Por: <Link href={`/posts/user/solo/discente/${id}`}><a className='list-group-item'>{discente.nome}</a></Link></p>
          </div>
          </div>
        ))}
